...
async function refreshMe(){
  ...
  $('#userPill').textContent=loggedIn?(isAdmin?`Signed in as ${loggedIn} (Admin)`:`Signed in as ${loggedIn}`):'';
...
function renderRobots(){
  const sorted=sortRobots(robotsCache);$('#robotCount').textContent=`${robotsCache.length} ${robotsCache.length===1?'bot':'bots'}`;$('#sortDirection').textContent=sortDesc?'↓':'↑';$('#sortDirection').title=sortDesc?'Descending':'Ascending';
  const grid=$('#grid');grid.innerHTML='';
  for(const bot of sorted){const el=document.createElement('article');el.className='card';el.innerHTML=`<img class="thumb" src="${esc(bot.thumbnail_url)}" alt="${esc(bot.name)}"><div class="card-body"><h3>${esc(bot.name)}</h3><div class="meta">by ${esc(bot.username)}</div><div class="card-actions"><button class="aboutBtn">About</button></div></div>`;el.querySelector('.aboutBtn').onclick=()=>showDetail(bot.id);grid.appendChild(el)}
}
...
$('#uploadBtn').onclick=()=>{
  if(!loggedIn){alert('Please log in or register to upload a bot.');return}
  $('#uploadError').textContent='';$('#uploadForm').reset();$('#cropPreview').src='';$('#previewFiles').value='';crop={image:null,scale:1,x:0,y:0,drag:false,lastX:0,lastY:0};$('#previewFileList').textContent='No preview images selected.';resizeCanvas();modal('#uploadModal')
};
...
$('#uploadForm').onsubmit=async e=>{e.preventDefault();$('#uploadError').textContent='';if(!crop.image){$('#uploadError').textContent='Please select a thumbnail.';return}try{const thumb=await makeCropBlob();const fd=new FormData();fd.append('name',$('#robotName').value);fd.append('description',$('#robotDescription').value);fd.append('bot',$('#botFile').files[0]);fd.append('thumbnail',thumb,'thumbnail.jpg');for(const f of [...$('#previewFiles').files])fd.append('previews',f,f.name);await api('/api/robots',{method:'POST',body:fd});modal('#uploadModal',false);await loadRobots()}catch(err){$('#uploadError').textContent=err.message}};
