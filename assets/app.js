...
function updateDynamicLanguage(){
  ...
  $('#sortDirection').title=sortDesc?'Descending':'Ascending';
...
function renderRobots(){
  const sorted=sortRobots(robotsCache);$('#robotCount').textContent=`${robotsCache.length} ${robotsCache.length===1?t('bot'):t('bots')}`;$('#sortDirection').textContent=sortDesc?'↓':'↑';$('#sortDirection').title=sortDesc?'Descending':'Ascending';
...
$('#languageBtn').onclick=()=>{if(typeof getLanguage!=='function'||typeof setLanguage!=='function') return;const next=getLanguage()==='en'?'ja':'en';setLanguage(next);};
arrangeAuthButtons();
...
$('#uploadForm').onsubmit=async e=>{e.preventDefault();$('#uploadError').textContent='';if(!crop.image){$('#uploadError').textContent=getLanguage()==='ja'?'サムネイルを選択してください。':'Please select a thumbnail.';return}try{const thumb=await makeCropBlob();const fd=new FormData();fd.append('name',$('#robotName').value);fd.append('description',$('#robotDescription').value);fd.append('bot',$('#botFile').files[0]);fd.append('thumbnail',thumb,'thumbnail.jpg');for(const f of [...$('#previewFiles').files])fd.append('previews',f,f.name);await api('/api/robots',{method:'POST',body:fd});modal('#uploadModal',false);await loadRobots()}catch(err){$('#uploadError').textContent=err.message}};
